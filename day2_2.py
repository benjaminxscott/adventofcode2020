...
def main():
    with open ('day2input.txt') as fp:
        ...
        for line in lines:
            ...
            index_1, index_2 = [int(x) -1 for x in indices]
            print(f"Checking for zero-based indices: {index_1}, {index_2} in {passwd}")

            try:
                # XOR on whether the char(s) at given index is correct
                if bool (passwd[index_1] == policy_char) ^ bool (passwd[index_2] == policy_char):
                    num_valid +=1

            # if the index is invalid, it's OK, implicitely not valid
            except IndexError:
                pass
